//
// Created by jkhu29 on 2021/7/10.
//
#include "lcd.h"
#include "stdlib.h"
#include "FONT.H"
...
// 重要参数
_lcd_dev lcddev;               // 默认为竖屏

// 写寄存器函数
// regval: 寄存器数值
void LCD_WR_REG(vu16 regval)
{
  regval = regval;             // -O2，用于延时
  LCD -> LCD_REG = regval;     // 写入要写的寄存器的序号
}
...
{
  LCD -> LCD_RAM = RGB_Code;        // 16位
}

// 从ILI93xx读出的数据位GBR格式，我们写入为RGB格式
// 通过该函数的转换
// c:GBR格式的颜色值
// 返回值:RGB格式
...
{
  vu16 r=0,g=0,b=0;
  if(x>=lcddev.width||y>=lcddev.height)return 0;	// 越界，返回
  LCD_SetCursor(x,y);
  if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310)LCD_WR_REG(0X2E);//9341/6804/3510 ·?????GRAM????
  else if(lcddev.id==0X5510)LCD_WR_REG(0X2E00);	// 5510 发送读GRAM指令
  else LCD_WR_REG(R34);                                 // 其他IC发送读取GRAM指令
...
void LCD_SetCursor(uint16_t Xpos, uint16_t Ypos)
{
  ...
  {
    if(lcddev.dir==1)Xpos=lcddev.width-1-Xpos;//横屏时处理，调换xy坐标
...
void LCD_Scan_Dir(uint8_t dir)
{
  ...
  if(lcddev.id==0x9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X5510)
  {
    ...
    if((lcddev.id!=0X5310)&&(lcddev.id!=0X5510))regval|=0X08;//5310/5510不需要RGB
...
void LCD_Init(void)
{
  ...
  if(lcddev.id<0XFF||lcddev.id==0XFFFF||lcddev.id==0X9300)
  {
    ...
    if(lcddev.id!=0X9341)	//不是9341,看看是不是6804
    {
      LCD_WR_REG(0XBF);
      lcddev.id=LCD_RD_DATA();//dummy read
      ...
      lcddev.id<<=8;
      lcddev.id|=LCD_RD_DATA();//读回0X04
      if(lcddev.id!=0X6804)    //不是6804,看看是不是NT35310，下同
      {
        LCD_WR_REG(0XD4);
  ...
  // TODO: 这里操作了FSMC
  if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510)//如果是这三个IC，设置WR最快
  {
    // 重新配置写时序控制寄存器的时序
    FSMC_Bank1E->BWTR[6]&=~(0XF<<0);// 地址建立时间清0
    FSMC_Bank1E->BWTR[6]&=~(0XF<<8);// 数据保存时间清0
  ...
  else if(lcddev.id==0x1505)//OK
  {
    ...
    LCD_WriteReg(0x0001,0x0100);
    LCD_WriteReg(0x0002,0x0700);
    LCD_WriteReg(0x0003,0x1038);//上到下，左到右
    LCD_WriteReg(0x0004,0x0000);
    LCD_WriteReg(0x000C,0x0000);
    LCD_WriteReg(0x000F,0x0000);
    LCD_WriteReg(0x0020,0x0000);
...
void LCD_Fill(uint16_t sx,uint16_t sy,uint16_t ex,uint16_t ey,uint16_t color)
{
  ...
  uint16_t xlen=0;
  uint16_t temp;
  if((lcddev.id==0X6804)&&(lcddev.dir==1))	//6804特殊
  {
    temp=sx;
    sx=sy;
...
void LCD_ShowChar(uint16_t x,uint16_t y,uint8_t num,uint8_t size,uint8_t mode)
{
  ...
  for(t=0;t<csize;t++)
  {
    if(size==12)temp=asc2_1206[num][t]; 	 	//1206字体
    else if(size==16)temp=asc2_1608[num][t];	//1608字体
    else if(size==24)temp=asc2_2412[num][t];	//2412字体
    else return;
    for(t1=0;t1<8;t1++)
    {
      ...
      temp<<=1;
      y++;
      if(y>=lcddev.height)return;	//越界
      if((y-y0)==size)
      {
        y=y0;
        x++;
        if(x>=lcddev.width)return;	//越界
        break;
      }
    }
  }
